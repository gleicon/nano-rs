...
use nano::sliver::{pack_sliver, unpack_sliver, validate_sliver, SliverMetadata};
...
fn test_sliver_migration_portability() {
    ...
    let vfs_entries = vec![
        (
            ...
            VfsFile::new(br#"{"app": "migration-test", "version": "1.0.0"}"#.to_vec()),
        ),
        (
            VfsPath::new("data/users.csv").unwrap(),
            VfsFile::new(
                b"id,name,email\n1,Alice,alice@example.com\n2,Bob,bob@example.com".to_vec(),
            ),
        ),
        (
            VfsPath::new("assets/logo.png").unwrap(),
            VfsFile::new(
    ...
    validate_sliver(&archive_a).expect("Instance A: Created sliver failed validation");
    ...
    let unpacked_b =
        unpack_sliver(&transferred_bytes).expect("Instance B: Failed to unpack sliver");
    ...
    assert_eq!(
        unpacked_b.metadata.hostname, hostname,
        "Hostname mismatch after migration"
    );
    assert_eq!(
        unpacked_b.metadata.format_version, metadata.format_version,
        "Format version mismatch"
    );
    assert_eq!(
        unpacked_b.metadata.name, metadata.name,
        "Sliver name mismatch"
    );
    assert_eq!(
        unpacked_b.metadata.description, metadata.description,
    ...
    for (i, (expected_path, expected_file)) in vfs_entries.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "VFS path mismatch at index {}",
            i
        );
        assert_eq!(
            actual_file.content,
            expected_file.content,
            "VFS content mismatch at path {}",
            expected_path.as_str()
        );
    }
    ...
    println!(
        "  - Bytecode ({} bytes): ✓",
        unpacked_b.bytecode.as_ref().map(|b| b.len()).unwrap_or(0)
    );
    println!(
        "  - VFS entries ({} files): ✓",
        unpacked_b.vfs_entries.len()
...
fn test_sliver_migration_empty_vfs() {
    ...
    let archive =
        pack_sliver(&metadata, Some(&heap_data), None).expect("Failed to pack empty sliver");

    // Transfer
    let unpacked = unpack_sliver(&archive).expect("Failed to unpack empty sliver");
...
fn test_sliver_migration_large_vfs() {
    ...
    // Transfer
    let unpacked = unpack_sliver(&archive).expect("Failed to unpack large sliver");
    ...
    println!(
        "✓ Large sliver migration ({} files, {} bytes) successful!",
...
fn test_sliver_migration_corrupted_data() {
    ...
    // Pack
    let archive = pack_sliver(&metadata, Some(&heap_data), None).expect("Failed to pack");
...
fn test_sliver_migration_cross_platform_paths() {
    ...
    let vfs_entries = vec![
        (
            VfsPath::new("config/app.json").unwrap(),
            VfsFile::new(b"{}".to_vec()),
        ),
        (
            VfsPath::new("data/2024/01/records.csv").unwrap(),
            VfsFile::new(b"date,value".to_vec()),
        ),
        (
            VfsPath::new("assets/images/logo.png").unwrap(),
            VfsFile::new(b"PNG".to_vec()),
        ),
    ];

    // Pack
    let archive =
        pack_sliver(&metadata, Some(&heap_data), Some(&vfs_entries)).expect("Failed to pack");

    // Transfer
    let unpacked = unpack_sliver(&archive).expect("Failed to unpack");

    // Verify all paths preserved correctly
    assert_eq!(unpacked.vfs_entries.len(), 3);

    let paths: Vec<&str> = unpacked
        .vfs_entries
        .iter()
        .map(|(p, _)| p.as_str())
        .collect();
