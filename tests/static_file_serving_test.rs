...
fn test_html_entrypoint_serves_file() {
    ...
    assert!(
        content.contains("Static File Test"),
    ...
    assert!(
        ...
        "HTML should reference content-type concept"
    );
}
...
fn test_css_file_content() {
    ...
    assert!(
        ...
        "CSS should contain font-family rule"
    );
...
fn test_css_file_extension() {
    ...
    assert_eq!(
        css_path.extension().unwrap(),
        "css",
        "File should have .css extension"
    );
}
...
fn test_js_file_content_not_executable_in_test() {
    ...
    assert!(
        ...
        "JS should contain console.log statement"
    );
    assert!(
        content.contains("not be executed"),
        "JS should indicate it's for serving"
    );
}
...
fn test_missing_file_not_present() {
    ...
    assert!(
        !nonexistent.exists(),
        "Non-existent file should not be present"
    );
}
...
fn test_non_standard_files_not_present() {
    ...
    assert!(
        ...
        "404.html should not be present"
    );
    assert!(
        !dir.join("error.html").exists(),
        "error.html should not be present"
    );
    assert!(
        !dir.join("missing.txt").exists(),
        "missing.txt should not be present"
    );
}
...
fn test_sliver_directory_packing() {
    ...
    assert!(
        result.is_ok(),
        "Should create sliver from directory: {:?}",
        result.err()
    );
...
fn test_sliver_directory_exists_with_correct_name() {
    ...
    assert!(
        static_files_dir().exists(),
        "Static files directory must exist for sliver tests"
    );
}
...
fn test_js_entrypoint_detection() {
    ...
    match js_type {
        ...
        _ => panic!(
            "JS file should be detected as JavaScript entrypoint, got {:?}",
            js_type
        ),
    }
}
...
fn test_html_entrypoint_detection() {
    ...
    match html_type {
        ...
        _ => panic!(
            "HTML file should be detected as StaticFile entrypoint, got {:?}",
            html_type
        ),
    }
}
...
fn test_directory_entrypoint_detection() {
    ...
    match dir_type {
        ...
        _ => panic!(
            "Directory should be detected as StaticDir entrypoint, got {:?}",
            dir_type
        ),
    }
}
...
fn test_entrypoint_type_variants() {
    ...
    assert!(
        matches!(js, nano::http::router::EntrypointType::JavaScript(_)),
        "JS should be JavaScript"
    );

    let mjs = detect_entrypoint_type("./app.mjs");
    assert!(
        matches!(mjs, nano::http::router::EntrypointType::JavaScript(_)),
        "MJS should be JavaScript"
    );

    let ts = detect_entrypoint_type("./app.ts");
    assert!(
        matches!(ts, nano::http::router::EntrypointType::JavaScript(_)),
        "TS should be JavaScript"
    );

    // Test static files
    let html = detect_entrypoint_type("./index.html");
    assert!(
        matches!(html, nano::http::router::EntrypointType::StaticFile(_)),
        "HTML should be StaticFile"
    );

    let css = detect_entrypoint_type("./style.css");
    assert!(
        matches!(css, nano::http::router::EntrypointType::StaticFile(_)),
        "CSS should be StaticFile"
    );

    let png = detect_entrypoint_type("./image.png");
    assert!(
        matches!(png, nano::http::router::EntrypointType::StaticFile(_)),
        "PNG should be StaticFile"
    );
}
...
fn test_content_type_detection_html() {
    ...
    assert!(
        content_type_from_ext("html").contains("text/html"),
        "HTML should have text/html content-type"
    );
    assert!(
        content_type_from_ext("htm").contains("text/html"),
        "HTM should have text/html content-type"
    );
}
...
fn test_content_type_detection_css() {
    ...
    assert!(
        content_type_from_ext("css").contains("text/css"),
...
fn test_content_type_detection_js() {
    ...
    assert!(
        content_type_from_ext("js").contains("application/javascript"),
        "JS should have application/javascript content-type"
    );
    assert!(
        content_type_from_ext("mjs").contains("application/javascript"),
        "MJS should have application/javascript content-type"
    );
}
...
fn test_content_type_detection_other() {
    ...
    assert!(
        content_type_from_ext("json").contains("application/json"),
    ...
    assert!(
        content_type_from_ext("txt").contains("text/plain"),
        "TXT should have text/plain content-type"
    );
}
...
fn test_html_references_css() {
    ...
    assert!(
        html_content.contains("style.css"),
...
fn test_static_dir_path_as_string() {
    ...
    assert!(
        !dir_str.is_empty(),
        "Directory path should be convertable to string"
    );
    assert!(
        dir_str.contains("static-files"),
        "Path should contain 'static-files'"
    );
}
