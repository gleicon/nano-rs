...
fn test_sliver_full_workflow_create_run_snapshot_restore() {
    ...
    block_on(isolate.vfs().write(
        "/package.json",
        br#"{"name": "test-app", "version": "1.0.0"}"#,
    ))
    ...
    // Execute the initialization script using V8 directly
    {
        ...
        let script = v8::Script::compile(ctx_scope, code, None).expect("Failed to compile script");
        script
            .run(ctx_scope)
            .expect("Failed to execute initialization");
    }
    ...
    block_on(
        isolate
            .vfs()
            .write("/data/cache.db", b"cached data from runtime session"),
    )
    ...
    for i in 0..5 {
        ...
        block_on(
            isolate
                .vfs()
                .write(&path, &format!("fake-image-data-{:04}", i).into_bytes()),
        )
    ...
    let mut vfs_snapshot = vec![
        (
            nano::vfs::VfsPath::new("index.js").unwrap(),
            nano::vfs::VfsFile {
        ...
        (
            nano::vfs::VfsPath::new("package.json").unwrap(),
            nano::vfs::VfsFile {
        ...
        (
            nano::vfs::VfsPath::new("data/user-settings.json").unwrap(),
            nano::vfs::VfsFile {
        ...
        (
            nano::vfs::VfsPath::new("data/cache.db").unwrap(),
            nano::vfs::VfsFile {
    ...
    let heap_data = create_snapshot_from_nano(isolate).expect("Failed to create heap snapshot");
    ...
    let sliver_data = pack_sliver(&metadata, Some(&heap_data), Some(&vfs_snapshot))
        .expect("Failed to pack sliver");

    std::fs::write(&sliver_path, &sliver_data).expect("Failed to write sliver file");
    ...
    // Read the sliver
    let sliver_data = std::fs::read(&sliver_path).expect("Failed to read sliver");

    // Unpack the sliver
    let unpacked = unpack_sliver(&sliver_data).expect("Failed to unpack sliver");
    ...
    let restored_isolate =
        NanoIsolate::from_snapshot(unpacked.bytecode.as_deref().unwrap_or_default(), vfs)
    ...
    assert!(
        settings_str.contains("dark"),
        "Settings should contain theme preference"
    );
    println!("  ✓ Settings file contents verified");

    let cache =
        block_on(restored_isolate.vfs().read("/data/cache.db")).expect("Cache file missing");
    let cache_str = String::from_utf8_lossy(&cache);
    assert!(
        cache_str.contains("cached data"),
        "Cache should contain runtime data"
    );
