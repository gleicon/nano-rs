...
| **Isolate lifecycle / memory** (`worker/pool.rs`, `memory_monitor.rs`) | recycle-after-N; per-isolate heap cap + OOM termination; soft eviction (recycle under memory pressure) | integration + unit tests | `adversarial_memory.rs`, `heap_limit_test.rs`; `eviction_trigger_matches_worker_recycle_condition` |
| **Cross-isolate LRU eviction** (`worker/eviction.rs`) | picking LRU victims across isolates under whole-process pressure | unit tests only — **not wired** | see Findings; roadmap item |
...
- **Eviction was inert; soft eviction now wired, cross-isolate LRU still roadmap.**
  `EvictionManager` and `MemoryMonitor` were complete and unit-tested but never
  constructed or invoked by the worker pool — the drain shape again — while docs
  claimed "Implemented 100%". Fixed the false claim, then wired the feasible half:
  `worker/pool.rs` now runs a per-worker `MemoryMonitor`, and recycles the isolate at
  Critical/Emergency pressure (soft eviction), recording a tenant pressure event
  (which also un-orphans the `pressure_events` counters). The cross-isolate LRU
  `EvictionManager` remains not wired (it needs cross-thread coordination of `!Send`
  isolates) — a roadmap item, kept because it is a real tested implementation.
